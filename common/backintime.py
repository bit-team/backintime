...
"""
This module is the entry point for the non-gui part of Back In Time.

See `startApp()` for the entry point. The module do parse commandline
arguments (via `argparse`) and initiate all jobs around snapshots.
"""
...
def createParsers(app_name='backintime'):
    ...
    configArgsParser.add_argument('--share-path',
                                  ...
                                  type=str,
                                  action='store',
                                  help='Write runtime data (locks, messages, '
                                       'log and mountpoints) to %(metavar)s.')
    ...
    commonArgsParser.add_argument('--quiet',
                                  action='store_true',
                                  help='Be quiet. Suppress messages '
                                       'on stdout.')

    # define arguments which are only used by snapshots-path,
    ...
    rsyncArgsParser.add_argument('--checksum',
                                 action='store_true',
                                 help='force to use checksum for checking '
    ...
    removeArgsParser.add_argument('SNAPSHOT_ID',
                                  ...
                                  action='store',
                                  nargs='*',
                                  help='ID of snapshots which should '
                                       'be removed.')
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="For backwards compatibility commands can also be used with "
               "trailing '--'. All listed arguments will work with all "
               "commands. Some commands have extra arguments. Run "
               "'%(app_name)s <COMMAND> -h' to see the extra arguments."
               % {'app_name': app_name})
    ...
    epilog = "Run '%(app_name)s -h' to get help for additional " \
             "arguments. " % {'app_name': app_name}
    epilogCommon = epilog + 'Additional arguments: --config, --debug, ' \
    ...
    description = 'Take a new snapshot in background only if the profile ' \
    ...
    description = 'Check the profiles configuration and install ' \
                  'crontab entries.'
    ...
    decodeCP.add_argument('PATH',
                          ...
                          action='store',
                          nargs='*',
                          help='Decode PATH. If no PATH is specified on '
                               'command line a list of filenames will be '
                               'read from stdin.')
    ...
    nargs = '*'
    aliases.append((command, nargs))
    description = "Remove snapshots and don't ask for confirmation " \
                  "before. Be careful!"
    ...
    restoreCP.add_argument('WHERE',
                           ...
                           action='store',
                           nargs='?',
                           help="Restore to WHERE. An empty argument '' "
                                "will restore to original destination.")

    restoreCP.add_argument('SNAPSHOT_ID',
                           type=str,
                           action='store',
                           nargs='?',
                           help='Which SNAPSHOT_ID should be used. This can '
                                'be a snapshot ID or an integer starting '
                                'with 0 for the last snapshot, 1 for the '
                                'second to last, ... the very first '
                                'snapshot is -1')

    restoreCP.add_argument('--delete',
                           action='store_true',
                           help='Restore and delete newer files which are '
                                'not in the snapshot. WARNING: deleting '
                                'files in filesystem root could break your '
                                'whole system!!!')

    backupGroup.add_argument('--local-backup',
                             action='store_true',
                             help='Create backup files before changing '
                                  'local files.')

    backupGroup.add_argument('--no-local-backup',
                             action='store_true',
                             help='Temporary disable creation of backup '
                                  'files before changing local files. This '
                                  'can be switched of permanently in '
                                  'Settings, too.')

    restoreCP.add_argument('--only-new',
                           action='store_true',
                           help='Only restore files which does not exist or '
...
def startApp(app_name='backintime'):
    ...
    if tools.usingSudo() \
       and os.getenv('BIT_SUDO_WARNING_PRINTED', 'false') == 'false':
        ...
        logger.warning(
            "It looks like you're using 'sudo' to start %(app)s. This will "
            "cause some troubles. Please use either 'sudo -i %(app_name)s' "
...
def argParse(args):
    ...
    def join(args, subArgs):
        ...
        for key, value in vars(subArgs).items():
            # only add new values if it isn't set already or if there
            # really IS a value
...
def printHeader():
    ...
    print('\n'
          ...
          'under certain conditions; type "backintime --license" '
          'for details.')
...
class PseudoAliasAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        Args:
            ...
            namespace (argparse.Namespace):   Namespace that should get
                                              modified
...
def aliasParser(args):
    ...
    """
    if not args.quiet:
        logger.info("Run command '%(alias)s' instead of "
                    "argument '%(replace)s' due to backwards compatibility."
...
def getConfig(args, check=True):
    ...
    logger.debug('profiles: %s' % ', '.join(
        '%s=%s' % (x, cfg.profileName(x)) for x in cfg.profiles()
    ))
...
def shutdown(args):
    ...
    if not instance.busy():
        logger.info('There is no active snapshot for profile %s. '
                    'Skip shutdown.' % profile)
...
def smartRemove(args):
    ...
    if enabled:
        ...
        logger.info('Smart Remove will remove {} snapshots'
                    .format(len(del_snapshots)))
