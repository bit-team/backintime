...
There is a `if __name__ == "__name__"` in this file for currently unknown
reasons.
"""
...
import mount
import progress
# import bcolors  flymake tells me it is unused
import snapshotlog
...
class Snapshots:
    ...
    def busy(self):
        instance = ApplicationInstance(
            self.config.takeSnapshotInstanceFile(), False)
    ...
    def uid(self, name, callback=None, backup=None):
        ...
        Args:
            ...
            backup (int):       UID wich will be used if the username is
        ...
        if name in self.uidCache:
            ...
            try:
                ...
                if backup:
                    uid = backup
                    msg = "UID for '%s' is not available on this system. " \
                          "Using UID %s from snapshot." % (name, backup)
    ...
    def gid(self, name, callback=None, backup=None):
        ...
        Args:
            ...
            callback (method):  callable which will handle a given message
            backup (int):       GID wich will be used if the groupname is
                                unknown on this machine
        ...
        if name in self.gidCache:
            ...
            try:
                ...
                if backup is not None:
                    gid = backup
                    msg = "GID for '%s' is not available on this system. " \
                          "Using GID %s from snapshot." % (name, backup)
                    logger.info(msg, self)
    ...
    def restorePermission(self, key_path, path, fileInfoDict, callback=None):
        ...
        'chown' to new owner and new group. If that fails (most probably
        because we are not running as root and normal user has no rights to
        change ownership of files) try to at least 'chgrp' to the new group.
        Finally 'chmod' the new mode.
        ...
        if uid != -1 or gid != -1:
            ...
            if uid != st.st_uid:
                ...
                self.restoreCallback(
                    ...
                    ok,
                    "chown %s %s : %s"
                    % (path.decode(errors='ignore'), uid, gid))
            ...
            if not ok and gid != st.st_gid:
                ...
                self.restoreCallback(
                    callback,
                    ok,
                    "chgrp %s %s" % (path.decode(errors='ignore'), gid))
    ...
    def restore(self,
        ...
        Args:
            ...
            paths (:py:class:`list`, :py:class:`tuple` or :py:class:`str`):
                                        ...
                                        a new rsync process. Permissions will
                                        be restored for all paths in one run
        ...
        if backup:
            cmd_prefix.extend(
                ('--backup', '--suffix=%s' % self.backupSuffix()))

        if delete:
            cmd_prefix.append('--delete')
            cmd_prefix.append(
                '--filter=protect %s' % self.config.snapshotsPath())
            cmd_prefix.append(
                '--filter=protect %s' % self.config._LOCAL_DATA_FOLDER)
            cmd_prefix.append(
                '--filter=protect %s' % self.config._MOUNT_ROOT)
        ...
        for path in paths:
            ...
            cmd.append(self.rsyncRemotePath(
                '%s.%s' % (src_base, src_path), use_mode=['ssh']))
        ...
        if fileInfoDict:
            ...
            for path, src_delta in restored_paths:
                ...
                if os.path.isdir(snapshot_path_to) \
                   and not os.path.islink(snapshot_path_to):
                    ...
                    for explore_path, dirs, files in os.walk(
                            snapshot_path_to.encode()):

                        for item in dirs:
                            item_path = os.path.join(
                                explore_path, item)[head:]
            ...
            self.restoreCallback(
                callback, True, _("Restore permissions:") + ' ' + status)
    ...
    def remove(self, sid):
        ...
        with TemporaryDirectory() as d:
            ...
            rsync.append(self.rsyncRemotePath(
    ...
    def backup(self, force=False):
        ...
        prohibing a new snapshot (e.g. on battery) and the profile is
        configured correctly. This will also mount and unmount remote
        destinations.
        ...
        if not self.config.isConfigured():
            ...
            if not instance.check():
                logger.warning(
                    ...
                    'running backup is in file %s.  Maybe delete it'
                    % instance.pidFile, self)
                ...
                logger.warning(
                    ...
                    'done. The pid of the already running restore is in %s. '
                    'Maybe delete it.' % restore_instance.pidFile, self)
            else:
                if self.config.noSnapshotOnBattery() \
                   and not tools.powerStatusAvailable():

                    logger.warning('Backups disabled on battery but '
                                   'power status is not available', self)
                ...
                if not include_folders:
                           ...
                           and self.config.notify():
                            ...
                            self.setTakeSnapshotMessage(
                                ...
                                _('Can\'t find snapshots folder.\nIf it is '
                                  'on a removable drive please plug it.')
                                + '\n'
                                + gettext.ngettext(
                                    'Waiting %s second.',
                                    'Waiting %s seconds.', 30) % 30,
                                30)
                    ...
                    if not self.config.canBackup(profile_id):
                        ...
                        if sid.exists():
                            logger.warning(
                                "Snapshot path \"%s\" already exists"
                                % sid.path(), self)
                            # This snapshots already exists
    ...
    def rsyncCallback(self, line, params):
        ...
        Args:
            line (str):     stdout line from rsync
            params (list):  list of two bool '[error, changes]'. Using
                            siteefect on changing list items will change
                            original list, too. If rsync reported an error
                            ``params[0]`` will be set to ``True``. If rsync
                            reported a changed
    ...
    def makeDirs(self, path):
        ...
        if not tools.makeDirs(path):
            ...
            self.setTakeSnapshotMessage(
                1, _('Can\'t create folder: %s') % path)
            # max 1 backup / second
            time.sleep(2)
    ...
    def backupConfig(self, sid):
        ...
        with open(self.config._LOCAL_CONFIG_PATH, 'rb') as src:
            ...
            if self.config.snapshotsMode() == 'local_encfs':
                ...
                local_encfs_path = os.path.join(
                    self.config.localEncfsPath(), 'config')

                with open(local_encfs_path, 'wb') as dst2:
                    dst2.write(src.read())
                ...
                cmd.append(
                    self.rsyncRemotePath(self.config.sshSnapshotsPath()))
    ...
    def backupInfo(self, sid):
        ...
        i.setListValue('user',
                       ('int:uid', 'str:name'),
                       list(self.userCache.items()))
        i.setListValue('group',
                       ('int:gid', 'str:name'),
                       list(self.groupCache.items()))
        i.setStrValue('filesystem_mounts',
                      json.dumps(tools.filesystemMountInfo()))
    ...
    def backupPermissions(self, sid):
        ...
        rsync.append(self.rsyncRemotePath(
            sid.pathBackup(use_mode=['ssh', 'ssh_encfs']))+os.sep)
    ...
    def backupPermissionsCallback(self, line, user_data):
        ...
        self.collectPermission(
            fileInfoDict, b'/' + decode.path(line).rstrip(b'/'))
    ...
    def takeSnapshot(self, sid, now, include_folders):
        ...
        if new_snapshot.exists() and new_snapshot.saveToContinue:
            ...
            logger.info("Found leftover '%s' which can be continued."
                        % new_snapshot.displayID, self)
            self.setTakeSnapshotMessage(
                0,
                _("Found leftover '%s' which can be continued.")
                % new_snapshot.displayID)
            ...
            logger.info("Remove leftover '%s' folder from last run"
                        % new_snapshot.displayID)

            self.setTakeSnapshotMessage(
                0,
                _("Removing leftover '%s' folder from last run")
                % new_snapshot.displayID)
            self.remove(new_snapshot)

            if os.path.exists(new_snapshot.path()):
                logger.error(
                    "Can't remove folder: %s" % new_snapshot.path(), self)
                self.setTakeSnapshotMessage(
                    1, _('Can\'t remove folder: %s') % new_snapshot.path())
                # max 1 backup / second
                time.sleep(2)
        ...
        # When there is no snapshots it takes the last snapshot from the
        # other folders
        ...
        try:
            ...
            logger.debug(
                'Failed to write takeSnapshot log %s into compressed '
                'file %s: %s'
        ...
        if not sid.exists():
            ...
            logger.error("Can't rename %s to %s"
                         % (new_snapshot.path(), sid.path()), self)
            self.setTakeSnapshotMessage(
                1,
                _('Can\'t rename %(new_path)s to %(path)s')
                % {
    ...
    def smartRemoveKeepAll(self,
        ...
        return set(
            [sid for sid in snapshots if sid >= min_id and sid < max_id])
    ...
    def smartRemoveList(self,
                        ...
                        keep_one_per_month):
        """
        Get a list of old snapshots that should be removed based on
        ...
        for sid in snapshots:
            ...
            if self.config.dontRemoveNamedSnapshots():
                ...
                if sid.name:
                    logger.debug(
                        "Keep snapshot: %s, it has a name" % sid, self)
                    continue
    ...
    def smartRemove(self, del_snapshots, log=None):
        ...
        Args:
            del_snapshots (list):   list of :py:class:`SID` that should be
                                    removed
            log (method):           callable method that will handle progress
                                    log
        """
        ...
        if self.config.snapshotsMode() in ['ssh', 'ssh_encfs'] \
           and self.config.smartRemoveRunRemoteInBackground():
            ...
            logger.info('[smart remove] remove snapshots in background: %s'
                        % del_snapshots, self)
            lckFile = os.path.normpath(os.path.join(
            ...
            # Eglot (lsp-server) and flymake are not able to handle that
            # strings with escape codes in it!
            # Check for unittests and try if that code realy does what it is
            # intended to do. Maybe an r-string could help here.
            ...
            for sid in del_snapshots:
                ...
                rsync = ' '.join(tools.rsyncRemove(
                    self.config, run_local=False))
                ...
                if logger.DEBUG:
                    ...
                    # add one second delay because otherwise you might not
                    # see serialized process with small snapshots
                    s += 'sleep 1; '
            ...
            for cmd in tools.splitCommands(
                    cmds,
                    head=head,
                    tail=tail,
    ...
    def freeSpace(self, now):
        ...
        if self.config.removeOldSnapshotsEnabled():
            ...
            oldBackupId = SID(self.config.removeOldSnapshotsDate(),
                              self.config)
            ...
            while True:
                ...
                logger.debug(msg.format(snapshots[0].withoutTag,
                                        oldBackupId.withoutTag),
                             self)
        ...
        if self.config.minFreeSpaceEnabled():
            self.setTakeSnapshotMessage(
                0, _('Trying to keep min free space'))

            minFreeSpace = self.config.minFreeSpaceMib()

            logger.debug("Keep min free disk space: {} MiB"
                         .format(minFreeSpace), self)
            ...
            while True:
                ...
                free_space = self.statFreeSpaceLocal(
                    self.config.snapshotsFullPath())
                ...
                logger.debug(
                    msg.format(free_space, snapshots[0].withoutTag), self)
                self.remove(snapshots[0])
        ...
        if self.config.minFreeInodesEnabled():
            ...
            self.setTakeSnapshotMessage(
                0, _('Trying to keep min %d%% free inodes') % minFreeInodes)
            logger.debug("Keep min {}%% free inodes"
                         .format(minFreeInodes), self)
            ...
            while True:
                ...
                try:
                    ...
                    logger.debug(
                        'Failed to get free inodes for snapshot path %s: %s'
    ...
    def statFreeSpaceLocal(self, path):
        ...
        :py:func:`os.statvfs()`. Depending on remote SFTP server this might
        fail on sshfs mounted shares.
    ...
    def filter(self,
        ...
        Args:
            ...
            list_diff_only (bool):  if ``True`` only return unique snapshots.
                                    Which means if a file is exactly the same
                                    in different snapshots only the first
                                    snapshot will be listed
    ...
    def flockExclusive(self):
        ...
        if self.config.globalFlock():
            ...
            if not s.st_mode & perms == perms:
                logger.debug(
                    'Set flock permissions %s' % self.GLOBAL_FLOCK, self)
    ...
    def rsyncSuffix(self, includeFolders=None, excludeFolders=None):
        ...
        encode = self.config.ENCODE
        ret = ['--chmod=Du+wx']
        ret.extend(['--exclude=' + i
                    for i in (encode.exclude(self.config.snapshotsPath()),
                              encode.exclude(self.config._LOCAL_DATA_FOLDER),
                              encode.exclude(self.config._MOUNT_ROOT)
                              )])

        # TODO: fix bug #561:
        # after rsync_exclude we need to explicite include files inside
        # excluded folders, recursive exclude folder-content again and
        # finally add the rest from rsync_include2
    ...
    def rsyncInclude(self, includeFolders=None):
        ...
        for include_folder in includeFolders:
            ...
            if folder == "/":
                # because an extra / is added below. Patch thanks
                # to Martin Hoefling
                # folder = ""
                items2.add('--include=/')
                items2.add('--include=/**')
...
class SID(object):
    ...
    Args:
        date (:py:class:`str`, :py:class:`datetime.date`
                                or :py:class:`datetime.datetime`):
                                used for creating this snapshot. str must be
                                in snapshot ID format (e.g 20151218-173512-123)
        cfg (config.Config):    current config
    ...
    def path(self, *path, use_mode=[]):
        ...
        if 'ssh' in use_mode and current_mode == 'ssh':
            return os.path.join(
                self.config.sshSnapshotsFullPath(self.profileID),
                self.sid,
                *path)
    ...
    def makeDirs(self, *path):
        ...
        if not os.path.isdir(self.config.snapshotsFullPath(self.profileID)):
            logger.error(
                'Snapshots path {} doesn\'t exist. Unable to make dirs for '
                'snapshot ID {}'
                .format(self.config.snapshotsFullPath(self.profileID),
                        self.sid),
                self)
    ...
    def canOpenPath(self, path):
        ...
        target = os.path.join(
            os.path.abspath(os.path.dirname(fullPath)),
            target)
    ...
    def lastChecked(self):
        ...
        if os.path.exists(info):
            return time.strftime('%Y-%m-%d %H:%M:%S',
                                 time.localtime(os.path.getatime(info)))

        return self.displayID

    # using @property.setter would be confusing here
    # as there is no value to give
    ...
    def fileInfo(self, d):
        ...
        try:
            ...
            logger.error(
                'Failed to write {}: {}'.format(self.FILEINFO, str(e)))
    ...
    def log(self, mode=None, decode=None):
        ...
        Args:
            ...
            decode (encfstools.Decode): instance used for decoding lines or
                                        ``None``
        ...
        try:
            ...
            msg = ('Failed to get snapshot log from {}:'
                   .format(logFile), str(e))
    ...
    def setLog(self, log):
        ...
        try:
            ...
            logger.error('Failed to write log into compressed file {}: {}'
                         .format(logFile, str(e)), self)
