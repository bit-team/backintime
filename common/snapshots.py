...
class Snapshots:
    ...
    def backup(self, force=False):
        ...
        if not force and not self.config.backupScheduled():
            logger.debug(f'Profile "{self.config.profileName()}" is not scheduled to run now. '
                'This profile uses the "Repeatedly (anacron)" schedule, so it will be checked again later.'
                )
